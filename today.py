...
def cache_builder(edges, comment_size, force_cache, loc_add=0, loc_del=0):
    ...
    repo_map = {line.split()[0]: i for i, line in enumerate(data)}

    for node in edges:
        repo_hash = hashlib.sha256(node['node']['nameWithOwner'].encode('utf-8')).hexdigest()
        if repo_hash in repo_map:
            index = repo_map[repo_hash]
            try:
                if int(data[index].split()[1]) != node['node']['defaultBranchRef']['target']['history']['totalCount']:
                    # if commit count has changed, update loc for that repo
                    owner, repo_name = node['node']['nameWithOwner'].split('/')
                    loc = recursive_loc(owner, repo_name, data, cache_comment)
                    data[index] = repo_hash + ' ' + str(node['node']['defaultBranchRef']['target']['history']['totalCount']) + ' ' + str(loc[2]) + ' ' + str(loc[0]) + ' ' + str(loc[1]) + '\n'
...
if __name__ == '__main__':
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2001, 7, 4))
